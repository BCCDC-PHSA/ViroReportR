...
#'
#' @details
#' The input must include daily case counts with at least 14 valid days. If
#' \code{smooth_data = TRUE}, the daily counts are smoothed before model fitting.
#' Forecasts are generated by simulating future daily cases based on the most
#' recent reproduction number estimates.
#'
...
#'
#' @return
#' A *data frame* of simulated daily forecasts with columns:
#' \describe{
#'   \item{date}{Forecasted date}
#'   \item{incidence}{Projected number of daily confirmed cases}
#'   \item{sim}{Simulation run number}
#' }
#'
