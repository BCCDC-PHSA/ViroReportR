...
#'
#' @param time_period_result element from list output generated by \code{forecast_time_period_epiestim}
#'
#' @return
#' @export
#'
...
cur_samples_with_quantile <-  cur_samples_with_quantile %>%
  left_join(cur_samples_with_week_date, by = "week_date") %>%
  rename(sim_draws = value)
...
create_1w_2w_df_epiestim <- function(time_period_results){
  ...
  results <- lapply(time_period_results, pred_samples_with_quantile_helper)
  return(results)
}
