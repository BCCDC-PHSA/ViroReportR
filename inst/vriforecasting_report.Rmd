...
plover_check <- tibble(plover_list = vri_data$plover_list,
                  vri = names(vri_data$plover_list),
  zero_prop = purrr::map(plover_list,
    ~ vriforecasting:::forecast_quality_precheck( data = .x, cutoff = 1)
  )
)

plover_check <- plover_check %>%
  dplyr::filter(zero_prop <  config$zero_threshold) %>%
  pull(vri)

phrdw_check <- tibble(phrdw_list = vri_data$phrdw_list,
                  vri = names(vri_data$phrdw_list),
  zero_prop = purrr::map(phrdw_list,
    ~ vriforecasting:::forecast_quality_precheck(data = .x, cutoff = 1)
  )
)

phrdw_check <- phrdw_check %>%
  dplyr::filter(zero_prop < config$zero_threshold) %>%
  pull(vri)

# Filter lists 
vri_data$plover_list <- vri_data$plover_list[names(vri_data$plover_list) %in% plover_check]
vri_data$phrdw_list <- vri_data$phrdw_list[names(vri_data$phrdw_list) %in% phrdw_check]

# Filter VRI name dictionary
disease_dict_plover <- unname(disease_dict[intersect(names(disease_dict), names(vri_data$plover_list))])

disease_dict_phrdw <- unname(disease_dict[intersect(names(disease_dict), names(vri_data$phrdw_list))])
```
...
# Fix names
names(plover_forecasts$forecasts) <- disease_dict_plover
names(phrdw_forecasts$forecasts) <- disease_dict_phrdw
```
...
for (vri in disease_dict_plover) {
