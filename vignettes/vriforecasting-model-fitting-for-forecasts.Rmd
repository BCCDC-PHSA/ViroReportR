...
weekly_plover_date_type, start_date = "2022-10-02", n_days = 14, type = "flu_a", time_period = "weekly", algorithm = "EpiEstim")
```

The forecasts can then be plotted at all the sliding windows used using the `plot` function which uses the output of `forecast_time_period`. The function takes in an object of of class `forecast_time_period`
...
```{r , dpi = 300, fig.align = "center", fig.height = 5, fig.width = 5, out.width= '80%'}
plot_validation(time_period_result_weekly, pred_horizon_str = "1 week ahead", pred_plot = "ribbon")
```
