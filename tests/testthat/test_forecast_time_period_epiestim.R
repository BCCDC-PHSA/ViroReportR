...
test_that("Start date that does not match dataset dates throws an error", {
 expect_error(forecast_time_period_epiestim(data = weekly_transformed_plover_data, start_date_str = "2022-10-03", n_days = 14, type = "flu_a", aggregate_week = TRUE),
...
test_that("Start date greater than or equal to end date throws an error", {
  expect_error(forecast_time_period_epiestim(data = weekly_transformed_plover_data, start_date_str = "2022-11-27", n_days = 14, type = "flu_a", aggregate_week = TRUE),
...
test_that("Error is thrown when n_days is not a multiple of 7 and aggregate_week is set to true", {
  expect_error(forecast_time_period_epiestim(data = weekly_transformed_plover_data, start_date_str = "2022-10-02", n_days = 3, type = "flu_a", aggregate_week = TRUE),
...
test_that("aggregate_week = TRUE argument works correctly to produce weekly quantiles", {
  expect_equal(length(forecast_time_period_epiestim(data = weekly_transformed_plover_data, start_date_str = "2022-10-02", n_days = 14, type = "flu_a",
...
test_that("n_days works correctly to produce number of daily quantiles (even if not multiple of 7)", {
  expect_equal(length(forecast_time_period_epiestim(data = weekly_transformed_plover_data, start_date_str = "2022-10-02", n_days = 3, type = "flu_a",
...
test_that("extend_model_data works correctly and dimension of list produced is correct", {
  expect_equal(length(forecast_time_period_epiestim(data = weekly_transformed_plover_data, start_date_str = "2022-10-02", n_days = 14, type = "flu_a",
                                             aggregate_week = TRUE)), nrow(weekly_transformed_plover_data) - 1)
})

test_that("extend_model_data indexes properly when start date is not first observation in dataset", {
  expect_equal(length(forecast_time_period_epiestim(data = weekly_transformed_plover_data, start_date_str = "2022-10-09", n_days = 14, type = "flu_a",
                                                    aggregate_week = TRUE)), nrow(weekly_transformed_plover_data) - 2)
})


# Cleaning up environment
rm(weekly_plover_data, weekly_transformed_plover_data, disease_type)
